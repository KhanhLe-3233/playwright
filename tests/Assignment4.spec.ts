...
test('TC001 - Verify Sort by Price', async ({ page }) => {
  ...
  const sortedPriceElements = await page.locator('.inventory_item_price').all();
  const sortedPricesAfter = [];
  for (const priceElement of sortedPriceElements) {
    const priceText = await priceElement.textContent();
    sortedPricesAfter.push(parseFloat(priceText.replace('$', '')));
  }

  // Check if the new price list is sorted
  const sortedList = [...sortedPricesAfter].sort((a, b) => a - b);
  expect(sortedPricesAfter).toEqual(sortedList);
});
